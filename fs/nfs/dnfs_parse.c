 ...
 * - Token count bounded to CONFIG_DNFS_MAX_REMOTE_ADDRS (default 32)
...
/**
 * dnfs_free_address_list - Free a parsed dnfs address list
 * @list:  The address list to free (may be NULL)
 *
 * Frees memory allocated during dnfs_parse_remoteaddrs().
...
/**
 * dnfs_parse_remoteaddrs - Parse remoteaddrs= mount option
 ...
 */
int dnfs_parse_remoteaddrs(struct nfs_fs_context *ctx, const char *value)
{
	 ...
	 */
	max_count = CONFIG_DNFS_MAX_REMOTE_ADDRS;
	if (count > max_count)
		return -E2BIG;
	...
	while (*p && list->count < count) {
		...
		if (!rpc_pton(&init_net, p, token_len,
			...
			pr_warn("NFS: dnfs: invalid remote address '%.*s'\n",
				(int)token_len, p);
			ret = -EINVAL;
			goto out_free;
		}
