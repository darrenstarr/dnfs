/*
 * test_dnfs_parse.c — Unit tests for dnfs option parsing code.
 *
 * This file compiles the kernel's dnfs_parse.c option parser in user
...
{
	if (ctx) {
		dnfs_free_address_list(ctx->nfs_server.dnfs_remoteaddrs);
		free(ctx);
	}
}
...
static void test_valid_ipv4_single(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1");
	TEST_ASSERT_EQ(ret, 0, "should parse single IPv4");
	TEST_ASSERT(ctx->nfs_server.dnfs_remoteaddrs != NULL,
		    "address list should be allocated");
	TEST_ASSERT_EQ(ctx->nfs_server.dnfs_remoteaddrs->count, 1,
		       "should have 1 address");

	struct sockaddr_in *sin =
		(struct sockaddr_in *)&ctx->nfs_server.dnfs_remoteaddrs->addrs[0];
...
static void test_valid_ipv4_multi(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1~10.0.0.2~10.0.0.3");
	TEST_ASSERT_EQ(ret, 0, "should parse multiple IPv4");
	TEST_ASSERT_EQ(ctx->nfs_server.dnfs_remoteaddrs->count, 3,
...
static void test_valid_ipv6_single(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "2001:db8::1");
	TEST_ASSERT_EQ(ret, 0, "should parse single IPv6");
	TEST_ASSERT_EQ(ctx->nfs_server.dnfs_remoteaddrs->count, 1,
		       "should have 1 address");

	struct sockaddr_in6 *sin6 =
		(struct sockaddr_in6 *)&ctx->nfs_server.dnfs_remoteaddrs->addrs[0];
...
static void test_valid_mixed(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1~2001:db8::1");
	TEST_ASSERT_EQ(ret, 0, "should parse mixed IPv4/IPv6");
	TEST_ASSERT_EQ(ctx->nfs_server.dnfs_remoteaddrs->count, 2,
...
static void test_null_value(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, NULL);
...
static void test_empty_string(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "");
...
static void test_invalid_address(void)
{
	...
	int ret = dnfs_parse_remoteaddrs(ctx, "999.999.999.999");
...
static void test_hostname_rejected(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "server.example.com");
...
static void test_consecutive_tildes(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1~~10.0.0.2");
...
static void test_max_addresses(void)
{
	...
	/* Build a string with CONFIG_DNFS_MAX_REMOTE_ADDRS addresses. */
	char buf[4096];
	int pos = 0;
	unsigned int i;

	for (i = 0; i < CONFIG_DNFS_MAX_REMOTE_ADDRS; i++) {
		if (i > 0)
			buf[pos++] = '~';
	...
	int ret = dnfs_parse_remoteaddrs(ctx, buf);
	TEST_ASSERT_EQ(ret, 0, "max addresses should parse");
	TEST_ASSERT_EQ(ctx->nfs_server.dnfs_remoteaddrs->count,
		       CONFIG_DNFS_MAX_REMOTE_ADDRS,
...
static void test_exceed_max_addresses(void)
{
	...
	for (i = 0; i < CONFIG_DNFS_MAX_REMOTE_ADDRS + 1; i++) {
		if (i > 0)
			buf[pos++] = '~';
		int n = snprintf(buf + pos, sizeof(buf) - pos,
				"10.0.%d.%d", i / 256, i % 256);
...
static void test_null_byte_truncates(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, evil);
...
/*
 * Test: dnfs_free_address_list with NULL is safe.
 */
static void test_free_null(void)
{
	dnfs_free_address_list(NULL);
	tests_passed++;
}
...
static void test_repeated_parse(void)
{
	...
	ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.2~10.0.0.3");
...
static void test_context_cleanup(void)
{
	...
	int ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1~10.0.0.2");
...
static void test_trailing_whitespace(void)
{
	...
	int ret = dnfs_parse_remoteaddrs(ctx, "10.0.0.1 ");
...
/*
 * Test: regression — the CONFIG_DNFS ifdef guards must compile
 * cleanly when DNFS is disabled (this test always runs because
 * we define our own CONFIG_DNFS_MAX_REMOTE_ADDRS).
 */
static void test_ifdef_clean(void)
{
	/* If we got here, the #if guards in the source code are
	 * syntactically valid with CONFIG_DNFS enabled. For a full
...
int main(void)
{
	...
	printf("CONFIG_DNFS_MAX_REMOTE_ADDRS = %d\n\n",
	       CONFIG_DNFS_MAX_REMOTE_ADDRS);
